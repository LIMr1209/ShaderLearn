Shader "Hidden/ImageEffects/Anisotropic Kuwahara Filter"
{
    Properties
    {
    ...
    {
        float4 tensor = SampleStructureTensor(input.texcoord);
        ...
        for (int n = 0; n < _FilterKernelSectors; n++)
        {
            float sigmaSq = abs(standardDeviations[n].r + standardDeviations[n].g + standardDeviations[n].b);
            float alpha = 1 / (1 + pow(255 * sigmaSq, 0.5f * _FilterSharpness));

            sumAlpha += alpha;
            sumAlphaWeights += alpha * weightedAverages[n];
        }
