...
Shader "Hidden/CameraMotionBlur"
{
    ...
    {
        return saturate(
            float4(tex2D(_MainTex, i.uv).x, abs(tex2D(_MainTex, i.uv).y), -tex2D(_MainTex, i.uv).xy) *
            _DisplayVelocityScale);
    }
    ...
    float4 ReconstructionDiscBlur(v2f i) : SV_Target
    {
        ...
        jitteredDir = max(abs(jitteredDir.xyxy), _MainTex_TexelSize.xyxy * _MaxVelocity * 0.5) * sign(jitteredDir.xyxy)
            * float4(1, 1, -1, -1);
        #else
        ...
        for (int l = 0; l < SmallDiscKernelSamples; l++)
        {
            ...
            float f = softDepthCompare(zx, zy);
            float b = softDepthCompare(zy, zx);
            float alphay = b * cone(x, y.xy, vx) + f * cone(y.xy, x, vy) + cylinder(y.xy, x, vy) * cylinder(x, y.xy, vx)
                * 2.0;
            ...
            alphay = b * cone(x, y.zw, vx) + f * cone(y.zw, x, vy) + cylinder(y.zw, x, vy) * cylinder(x, y.zw, vx) *
                2.0;
